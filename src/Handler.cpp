...
ssize_t Handler::read_until(size_t size, std::vector<char> &buf) {
  return recv(size, buf.data(), buf.size(), 0);
}

std::vector<std::string> Handler::handle(std::stringstream &ss) {
  char b;
  ss.read(&b, 1);
  if ('*' == b) {
...
void Handler::read_delim(std::stringstream &ss) {
  char c[2];
  ss.read(c, 2);
}

std::vector<std::string> Handler::handle_aggregate(std::stringstream &ss) {
  int size;
  ss >> size;
  read_delim(ss);
...
void Handler::handle_set_command(std::vector<std::string> &req,
                                 std::stringstream &out) {
...
void Handler::resp(std::stringstream &out, const BulkString &bulk_string) {
...
void Handler::resp(std::stringstream &out, const SimpleString &simple_string) {
  out << '+';
  out << simple_string.body << "\r\n";
}

void Handler::resp(std::stringstream &out, const SimpleError &simple_error) {
  out << '-' << simple_error.body << "\r\n";
}

std::vector<std::string> Handler::handle_bulk_string(std::stringstream &ss) {
  int size;
  ss >> size;
  std::string word;
  word.resize(size);
...
Handler::Handler(int client_socket, std::shared_ptr<Storage> storage) {
...
void Handler::handle() {
  ...
  while (size_t size = recv(_client_socket, buffer.data(), buffer.size(), 0)) {
    std::stringstream ss{};
